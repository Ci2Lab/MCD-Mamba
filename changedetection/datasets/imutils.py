...
# def normalize_img_custom(img):
#     """Normalize image by subtracting mean and dividing by std along the channels."""
#     img = np.float32(img)
...
def normalize_img_z(img):
    ...
    # lower_bound = np.percentile(img, 2, axis=(0, 1))
    # upper_bound = np.percentile(img, 98, axis=(0, 1))
    
    # # Clip the image values
    # img_clipped = np.clip(img, lower_bound, upper_bound)
    
    # Calculate mean and std of clipped image
    mean = np.mean(img, axis=(0, 1))
    std = np.std(img, axis=(0, 1))
...
def normalize_img_custom(img):
    ...
    for i in range(img.shape[2]):  # Iterate over each channel
        channel = img[:,:,i]
        
        # Calculate 2nd and 98th percentiles
        lower_bound = np.percentile(channel, 2)
        upper_bound = np.percentile(channel, 98)
        
        # Clip the channel values
        channel_clipped = np.clip(channel, lower_bound, upper_bound)
        
        # Scale to 0-1 range
        channel_normalized = (channel_clipped - lower_bound) / (upper_bound - lower_bound)
        
        normalized_img[:,:,i] = channel_normalized
...
def random_crop(img, crop_size, mean_rgb=[0, 0, 0,0], ignore_index=255):
    h, w, _ = img.shape
...
def random_bi_image_crop(pre_img, object, crop_size, mean_rgb=[0, 0, 0,0], ignore_index=255):
    h, w = object.shape
...
def random_crop_new(pre_img, post_img, label, crop_size, mean_rgb=[0, 0, 0,0], ignore_index=255):
    h, w = label.shape

    H = max(crop_size, h)
    W = max(crop_size, w)

    pad_pre_image = np.zeros((H, W, 4), dtype=np.float32)

    pad_post_image = np.zeros((H, W, 4), dtype=np.float32)
