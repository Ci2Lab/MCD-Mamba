...
class ChangeDecoder(nn.Module):
    def __init__(self, encoder_dims, channel_first, norm_layer, ssm_act_layer, mlp_act_layer, **kwargs):
        ...
        self.st_block_4 = nn.Sequential(
            nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-1], out_channels=64),
        ...
        self.st_block_3 = nn.Sequential(
            nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-2], out_channels=64),
            Permute(0, 2, 3, 1) if not channel_first else nn.Identity(),
            VSSBlock(hidden_dim=64, drop_path=0.1, norm_layer=norm_layer, channel_first=channel_first,
                ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'], ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                ssm_conv=kwargs['ssm_conv'], ssm_conv_bias=kwargs['ssm_conv_bias'], ssm_drop_rate=kwargs['ssm_drop_rate'], ssm_init=kwargs['ssm_init'],
        ...
        self.st_block_2 = nn.Sequential(
            nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-3], out_channels=64),
            Permute(0, 2, 3, 1) if not channel_first else nn.Identity(),
            VSSBlock(hidden_dim=64, drop_path=0.1, norm_layer=norm_layer, channel_first=channel_first,
                ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'], ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                ssm_conv=kwargs['ssm_conv'], ssm_conv_bias=kwargs['ssm_conv_bias'], ssm_drop_rate=kwargs['ssm_drop_rate'], ssm_init=kwargs['ssm_init'],
        ...
        self.st_block_1 = nn.Sequential(
            nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-4] , out_channels=64),
            Permute(0, 2, 3, 1) if not channel_first else nn.Identity(),
            VSSBlock(hidden_dim=64, drop_path=0.1, norm_layer=norm_layer, channel_first=channel_first,
                ssm_d_state=kwargs['ssm_d_state'], ssm_ratio=kwargs['ssm_ratio'], ssm_dt_rank=kwargs['ssm_dt_rank'], ssm_act_layer=ssm_act_layer,
                ssm_conv=kwargs['ssm_conv'], ssm_conv_bias=kwargs['ssm_conv_bias'], ssm_drop_rate=kwargs['ssm_drop_rate'], ssm_init=kwargs['ssm_init'],
        ...
        # Fuse layer  
        self.fuse_layer_4 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=64 * 6, out_channels=64),
                                          nn.BatchNorm2d(64), nn.ReLU())
        self.fuse_layer_3 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=64 * 6, out_channels=64),
                                          nn.BatchNorm2d(64), nn.ReLU())
        self.fuse_layer_2 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=64 * 6, out_channels=64),
                                          nn.BatchNorm2d(64), nn.ReLU())
        self.fuse_layer_1 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=64 * 6, out_channels=64),
                                          nn.BatchNorm2d(64), nn.ReLU())

        # Smooth layer
        self.smooth_layer_3 = ResBlock(in_channels=64, out_channels=64, stride=1) 
        self.smooth_layer_2 = ResBlock(in_channels=64, out_channels=64, stride=1) 
        self.smooth_layer_1 = ResBlock(in_channels=64, out_channels=64, stride=1) 
    ...
    def forward(self, pre_features, post_features):
        ...
        '''
            Stage I
        '''
        p4 = self.st_block_4(pre_feat_4 - post_feat_4)
        # p41_2 = self.st_block_41_2(torch.cat([pre_feat_4, post_feat_4], dim=1))
        ...
        # p4 = self.fuse_layer_4(torch.cat([p41, p41_2, p42[:, :, :, ::2], p42[:, :, :, 1::2], p43[:, :, :, 0:W], p43[:, :, :, W:]], dim=1))
       

        '''
            Stage II
        '''
        p3 = self.st_block_3(pre_feat_3 - post_feat_3)
        # p31_2 = self.st_block_31_2(torch.cat([pre_feat_3, post_feat_3], dim=1))
        ...
        # p3 = self.fuse_layer_3(torch.cat([p31,p31_2, p32[:, :, :, ::2], p32[:, :, :, 1::2], p33[:, :, :, 0:W], p33[:, :, :, W:]], dim=1))
        ...
        '''
            Stage III
        '''
        p2 = self.st_block_2(pre_feat_2 - post_feat_2)
        # p21_2 = self.st_block_21_2(torch.cat([pre_feat_2, post_feat_2], dim=1))
        ...
        # p2 = self.fuse_layer_2(torch.cat([p21,p21_2, p22[:, :, :, ::2], p22[:, :, :, 1::2], p23[:, :, :, 0:W], p23[:, :, :, W:]], dim=1))
        ...
        '''
            Stage IV
        '''
        p1 = self.st_block_1(pre_feat_1 - post_feat_1)
        # p11_2 = self.st_block_11_2(torch.cat([pre_feat_1, post_feat_1], dim=1))
        ...
        # p1 = self.fuse_layer_1(torch.cat([p11, p11_2, p12[:, :, :, ::2], p12[:, :, :, 1::2], p13[:, :, :, 0:W], p13[:, :, :, W:]], dim=1))
